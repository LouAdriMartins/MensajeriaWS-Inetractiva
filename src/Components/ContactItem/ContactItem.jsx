...
export default function ContactItem({ id, name, image_route, last_message, hora, unread_messages, is_archived }) {
    return (
        <div className={`contact-item ${unread_messages > 0 ? 'has-unread' : ''} ${is_archived ? 'is-archived' : ''}`}>
            {is_archived ? (
                ...
                <Link to={`/contact/${id}/messages`} className="contact-link">
                    <div className="contact-avatar">
                        <img src={image_route || "/placeholder.svg"} alt={`Imagen de contacto ${name}`} className="avatar-image" />
                    </div>
                    <div className="contact-info">
                        <div className="contact-header">
                            <h3 className="contact-name">{name}</h3>
                        ...
                        <div className="contact-message">
                            <div className="message-content">
                                ...
                                <p className="message-text">{last_message.text}</p>
                            </div>
                            {unread_messages > 0 && (
                                <span className="unread-badge">{unread_messages > 99 ? "99+" : unread_messages}</span>
                            )}
                        </div>
                    </div>
                </Link>
